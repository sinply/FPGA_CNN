			...
			for(int j=0;j<Hout;j++)
			{
				...
				for(int ii=0;ii<Kx;ii++)
					for(int jj=0;jj<Ky;jj++)
					{
						...
						switch(mode)
						{
							case 0:{sum+=feature_in[c*Win*Hin+w*Hin+h];break;}
							case 1:{sum=min(sum,feature_in[c*Win*Hin+w*Hin+h]);break;}
							case 2:{sum=max(sum,feature_in[c*Win*Hin+w*Hin+h]);break;}
							default:break;
						}
					}
				if(mode==0)
					sum=sum/(Kx*Ky);
				feature_out[c*Wout*Hout+i*Hout+j]=sum;
			}
}
